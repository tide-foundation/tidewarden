...
make_config! {
    ...
    /// TideCloak settings |> Settings loaded automatically from data/tidecloak.json (generated by scripts/init-tidecloak.sh)
    tidecloak {
        /// Enabled |> Automatically set to true when data/tidecloak.json exists
        tide_enabled:                   bool,   true,   def,    false;
        /// Vendor ID |> Read from tidecloak.json vendorId field
...
/// Load TideCloak settings from the TIDECLOAK_JSON env var or data/tidecloak.json file.
/// This replaces the TIDE_* environment variables with values from the adapter config
/// generated by the TideCloak init script.
fn load_tidecloak_config(builder: &mut ConfigBuilder) {
    let (content, source) = if let Some(env_val) = get_env("TIDECLOAK_JSON") {
        (env_val, "TIDECLOAK_JSON env var".to_string())
    } else {
        let data_folder = get_env("DATA_FOLDER").unwrap_or_else(|| String::from("data"));
        let path = format!("{data_folder}/tidecloak.json");
    ...
    let json: serde_json::Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => {
            println!("[WARNING] Failed to parse TideCloak config from {source}: {e}");
            return;
        }
    };

    println!("[INFO] Loading TideCloak configuration from {source}.\n");

    builder.tide_enabled = Some(true);

    if let Some(v) = json.get("vendorId").and_then(|v| v.as_str()) {
        builder.tide_vendor_id = Some(v.to_string());
    }
    if let Some(v) = json.get("homeOrkUrl").and_then(|v| v.as_str()) {
        builder.tide_home_ork_url = Some(v.to_string());
    }
    if let Some(v) = json.get("realm").and_then(|v| v.as_str()) {
        builder.tide_realm = Some(v.to_string());
    }

    // Derive SSO_AUTHORITY from auth-server-url + realm if not already set
    if builder.sso_authority.is_none() {
        if let (Some(auth_url), Some(realm)) = (
            json.get("auth-server-url").and_then(|v| v.as_str()),
            json.get("realm").and_then(|v| v.as_str()),
        ) {
            let authority = format!("{}/realms/{}", auth_url.trim_end_matches('/'), realm);
            builder.sso_authority = Some(authority);
        }
    }

    // Derive SSO_CLIENT_ID from resource if not already set
    if builder.sso_client_id.is_none() {
        if let Some(v) = json.get("resource").and_then(|v| v.as_str()) {
            builder.sso_client_id = Some(v.to_string());
        }
    }

    // Look up client-origin-auth for the configured domain
    if let Some(domain) = &builder.domain {
        let key = format!("client-origin-auth-{domain}");
...
impl Config {
    pub async fn load() -> Result<Self, Error> {
        ...
        let mut _env = ConfigBuilder::from_env();

        // Load TideCloak settings from tidecloak.json (replaces TIDE_* env vars)
        load_tidecloak_config(&mut _env);
