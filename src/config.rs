...
/// Load TideCloak settings from data/tidecloak.json if it exists.
...
fn load_tidecloak_config(builder: &mut ConfigBuilder) {
    ...
    let json: serde_json::Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => {
            println!("[WARNING] Failed to parse {path}: {e}");
            return;
        }
    };

    println!("[INFO] Loading TideCloak configuration from `{path}`.\n");
